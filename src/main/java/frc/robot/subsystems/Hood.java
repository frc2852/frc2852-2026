...
public class Hood extends SubsystemBase {
  ...
  /**
   * Applies a small positive voltage to test motor direction. Watch the dashboard: if position INCREASES, direction is
   * correct. If position DECREASES, flip motor inversion.
   */
  ...
  /**
   * Hot-reload only Slot0 and MotionMagic gains. Uses per-group config objects so gear ratio, soft limits, and current
   * limits are never touched. Motion Magic params are in deg/s (matching HoodConstants convention), divided by 360
   * internally.
   */
